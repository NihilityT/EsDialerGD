...
void fuck(u_char *data, size_t len) {
    if (len > sizeof(magic_box)) return;
    for (size_t i = 0; i < len; ++i) data[i] ^= magic_box[i];
}

void bin2hex(char *hex, const u_char *bin, size_t bin_len) {
    for (size_t i = 0; i < bin_len; ++i) {
        char hexbyte[4] = {0};
        snprintf(hexbyte, sizeof(hexbyte), "%02X", bin[i]);
        hex[(i << 1)] = hexbyte[0];
        hex[(i << 1) + 1] = hexbyte[1];
    }
}
...
void hex2bin(u_char *bin, const char *hex) {
    ...
    for (size_t i = 0, p = 0; i < len; i += 2, ++p) sscanf(hex + i, "%02X", &bin[p]);
}
